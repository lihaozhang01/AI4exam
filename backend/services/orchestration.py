...
async def grade_and_save_test(
    ...
    for user_answer in request.answers:
        ...
        if question.question_type == 'essay':
            evaluation_result = await ai.evaluate_essay_with_ai(
                schemas.EvaluateShortAnswerRequest(
                    question_text=question.stem,
                    user_answer=user_answer.answer_text,
                    reference_answer=question.correct_answer.get('reference_explanation', '') if isinstance(question.correct_answer, dict) else question.correct_answer
                ),
                provider=provider,
                api_key=api_key,
    ...
    db_result = models.TestPaperResult(
        test_paper_id=int(request.test_id),
