...
def grade_and_save_test(db: Session, test_id: int, user_answers: List[schemas.UserAnswer]):
    ...
    test_paper = database.get_test_paper_by_id(db, test_id)
    ...
    for question in test_paper.questions:
        if question.question_type in grading.GRADING_STRATEGIES:
    ...
    for user_answer in user_answers:
        ...
        # 如果是論述題，直接提取參考答案，不進行評分
        if question.question_type == 'essay':
            explanation = ""
            # 參考答案儲存在 correct_answer['reference_explanation'] 中
        ...
        # 處理客觀題
        if question.question_type not in grading.GRADING_STRATEGIES:
            continue
...
async def generate_and_save_overall_feedback(
    ...
    feedback = await ai.get_overall_feedback_from_ai(graded_info, generation_model, generation_prompt)
...
async def generate_and_save_single_question_feedback(
    ...
    feedback = await ai.get_single_question_feedback_from_ai(question, user_answer, generation_model, generation_prompt)
